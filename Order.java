...
public class Order {
    ...
    public static void pizzaFinisher(ArrayList<Pizza> Display,ArrayList<Menu> pizza) throws IOException {
        if (!Display.isEmpty()) {
    ...
    public static void secondStageFinisher(ArrayList<Pizza> Display,ArrayList<Menu> pizza) throws IOException {
        ...
        if (Display.get(orderNumber-1).isActive()) {
            ...
            /*
            System.out.println("Archive was empty! starting from scratch");
            Files.writeArchive(Display, pizza, orderNumber - 1, orderNumber-1);
            */
            System.out.println("\nOrder " + Display.get(orderNumber - 1).getOrderNumber() + " is now finished and archived in the system...\n");
        } else {
            System.out.println("This order is already completed & Archived!\nTry typing another Order number\n");
        }
    }
}
